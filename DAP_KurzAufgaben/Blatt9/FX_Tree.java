...
public class FX_Tree {
    ...
    public void apply_FX(Group grp) {
        ...
        Text t = new Text(baseX - 14, baseY + 5, "" + root);
    ...
    public static <T> FX_Tree createFromTree(T root, Function<T, T> left, Function<T, T> right, Function<T, Integer> value) {
        ...
        tree.setLeft(createFromTree(left.apply(root), left, right, value));
        tree.setRight(createFromTree(right.apply(root), left, right, value));
        return tree;
    }

    public static FX_Tree createFrom(Tree root) {
        if (!root.empty) return createFromTree(root, (a) -> a.leftChild, (a) -> a.rightChild, (a) -> a.value);
        else return null;
    }
}
